...
import type {
  AgentAdapter, AgentCapabilities, AgentContext, AgentHealth, AgentRequest, AgentResponse, AgentSession,
  AgentSessionOptions, MessagePart, RegisteredAgent,
...
interface RemotePeerState { cardUrl: string; card: AgentCard; trustStatus: PeerTrust; }
interface RemoteSessionState { agentId: string; options: AgentSessionOptions; taskId?: string; }

export interface A2aRemoteAdapterOptions { nodeId: string; events: EventStore; driver?: A2aClientDriver; }

/** Makes remote A2A agents routable through the same AgentAdapter contract as local CLIs and ACP agents. */
export class A2aRemoteAdapter implements AgentAdapter {
  ...
  async send(session: AgentSession, request: AgentRequest, context: AgentContext): Promise<AgentResponse> {
    ...
    try {
      ...
      state.taskId = "id" in result ? result.id : (result.taskId || taskId);
      this.options.events.publish("federation.task_sent", {
        agentId: state.agentId,
        remoteTaskId: state.taskId,
        contextId: context.conversationId,
  ...
  async cancel(executionId: string): Promise<void> {
    ...
    await this.driver.cancelTask(this.requirePeer(state.agentId).card, state.taskId);
  }
  ...
  async resolveAndStorePeer(agentId: string, cardUrl: string, trustStatus: PeerTrust, signal?: AbortSignal): Promise<AgentCard> {
    ...
    if (!card.supportedInterfaces.some((entry) => entry.protocolVersion === A2A_PROTOCOL_VERSION)) throw new Error(`A2A peer ${agentId} does not advertise protocol ${A2A_PROTOCOL_VERSION}`);
...
export async function registerRemoteA2aPeer(options: RegisterRemoteA2aPeerOptions): Promise<RegisteredAgent> {
  ...
  const card = await options.adapter.resolveAndStorePeer(agentId, options.cardUrl, trustStatus, options.signal);
  ...
  return options.registry.register(agent);
}

function a2aResultToAgentResponse(result: SendMessageResult): AgentResponse {
  if ("messageId" in result) return { content: a2aPartsToInternal(result.parts), artifacts: [], vendorMessageId: result.messageId };
  const content: MessagePart[] = [];
  const status = result.status;
  if (status?.message) content.push(...a2aPartsToInternal(status.message.parts));
  for (const artifact of result.artifacts) content.push(...a2aPartsToInternal(artifact.parts));
  if (!content.length) content.push({ type: "json", value: { taskId: result.id, state: status?.state } });
